...
void FDCAN_Recieve_RunDataFromLiftMotors(uint8_t functionID,uint8_t motorID)
{
	...
	if(functionID==LIFTRUNTIMEDATA_FUNCTIONID){
		...
		R[motorID].currentRaw= (RxData[10]<< 8 ) | RxData[11];
		R[motorID].voltageRaw= (RxData[8]<< 8 ) | RxData[9];
...
void FDCAN_Recieve_HomingDataFromLiftMotors(uint8_t source_address)//,LiftRunTime_TypeDef *LR)
{
	...
	if (data == HOMING_DONE){
		if (source_address == RD_LEFTLIFT_ADDRESS){
			S.HomingDoneCounter += 1;
		}else if (source_address == RD_RIGHTLIFT_ADDRESS){
			S.HomingDoneCounter += 1;
		}
...
void FDCAN_Recieve_LiftChangeDirection(uint8_t source)
{
	...
	if(SO.liftStrokeOverMsgsRecieved==2){
		...
		if (mcParams.currentLiftDirection == LIFT_DOWN){
			mcParams.left_liftStrokeHeight = R[RD_LEFT_LIFT].presentPosition - mcParams.currentStrokeTargetAbsPos;
			mcParams.right_liftStrokeHeight = R[RD_RIGHT_LIFT].presentPosition - mcParams.currentStrokeTargetAbsPos;
		}else{
			mcParams.left_liftStrokeHeight =  mcParams.currentStrokeTargetAbsPos - R[RD_LEFT_LIFT].presentPosition ;
			mcParams.right_liftStrokeHeight = mcParams.currentStrokeTargetAbsPos - R[RD_RIGHT_LIFT].presentPosition ;
		}
...
void FDCAN_sendNewStroke_ToBothLiftMotors(machineParamsTypeDef *mp)
{
	...
	TxData[2]=((uint16_t)(mp->currentStrokeTime*100))>>8;
	TxData[3]=(uint16_t)(mp->currentStrokeTime*100);
	TxData[4]=mp->currentLiftDirection;

	//send data to left lift
	TxHeader.Identifier =(0xA15<<16)|(RD_LEFTLIFT_ADDRESS<<8)|0x01;
	TxHeader.DataLength = FDCAN_DLC_BYTES_6;
	while(HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan1)==0){};
	HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &TxHeader, TxData);
	...
	TxHeader.Identifier =(0xA15<<16)|(RD_RIGHTLIFT_ADDRESS<<8)|0x01;
	TxHeader.DataLength = FDCAN_DLC_BYTES_6;
	while(HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan1)==0){};
	HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &TxHeader, TxData);
}
