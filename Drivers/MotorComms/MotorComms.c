...
void ReadySetupCommand_AllMotors(machineSettingsTypeDef *ms,machineParamsTypeDef *mp){
	...
	SU[0].RPM = mp->calenderMotorRPM;//mp->flyerMotorRPM;
	SU[0].RDT = mp->rampTimes_sec;
	SU[0].RUT = mp->rampTimes_sec;


	SU[1].StrokeLength = mp->left_liftStrokeHeight;
	SU[1].StrokeTime = mp->currentStrokeTime;
	SU[2].StrokeLength = mp->right_liftStrokeHeight;
	SU[2].StrokeTime = mp->currentStrokeTime;
	for (int i=1;i<3;i++){
		SU[i].direction = LIFT_UP;
		SU[i].liftRUT = mp->rampTimes_sec; // these are two are sent in seconds
		SU[i].liftRDT = mp->rampTimes_sec;
		SU[i].liftCRT = mp->changeLayerTimes_ms; // this is sent as uint16
	}
...
uint8_t  SendCommands_To_MultipleMotors(uint8_t *motorList,uint8_t motorArraySize,uint8_t command){
	...
	if (command == START){
		...
		for (int i=0;i<noOfMotors;i++){
			...
			motor = motorList[i];
			if ((canID != RD_LEFTLIFT_ADDRESS) && (canID != RD_RIGHTLIFT_ADDRESS)){
