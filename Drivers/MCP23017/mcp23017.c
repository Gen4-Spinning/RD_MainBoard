...
void mcp23017_setupForMainBoard(MCP23017_HandleTypeDef *hdev){
	HAL_StatusTypeDef status0;
	 ...
	 data[0] = IODIRA_DEFAULT;
	 status0 = mcp23017_write(hdev, IODIRA, data);
	 HAL_Delay(2);

	 data[0] = IOPOLA_DEFAULT;
	 status0 = mcp23017_write(hdev, IPOLA, data);
	 HAL_Delay(2);

	 data[0] = GPPUA_DEFAULT;
	 status0 = mcp23017_write(hdev, GPPUA, data);
	 HAL_Delay(2);
