...
uint8_t motors[] = {CALENDER_ROLLER};
...
void DebugState(void){
	...
	while(1){
		...
		if(requestGBDOnce)
		{
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_LEFTLIFT_ADDRESS, ONESHOT);
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_RIGHTLIFT_ADDRESS, ONESHOT);
			requestGBDOnce=0;
		}
		if(requestGBDForever)
		{
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_LEFTLIFT_ADDRESS, CONTINUOUS);
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_RIGHTLIFT_ADDRESS, CONTINUOUS);
			requestGBDForever=0;
		}
		if(stopGBD)
		{
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_LEFTLIFT_ADDRESS, CONTINUOUS_STOP);
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_RIGHTLIFT_ADDRESS, CONTINUOUS_STOP);
			stopGBD=0;
		}

		if(fixCurrentPosAsHomeLeft){
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_LEFTLIFT_ADDRESS, SETHOMINGPOS);
			S.oneSecTimer = 0;
		...
		if(fixCurrentPosAsHomeRight){
			FDCAN_sendGBCalibCommand_ToLiftMotor(RD_RIGHTLIFT_ADDRESS, SETHOMINGPOS);
			S.oneSecTimer = 0;
			while(S.oneSecTimer < 1){};
			if (GBD.homeSetupSuccess){
				//send BT success msg
