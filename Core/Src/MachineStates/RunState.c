...
void RunState(void){
	...
	while(1){
		...
		if (S.oneTime){
			...
			uint8_t motors[] = {CALENDER_ROLLER,RD_LEFT_LIFT,RD_RIGHT_LIFT};//{FLYER,BOBBIN,FR,BR};//,LEFT_LIFT,RIGHT_LIFT};
			noOfMotors = 3;
		...
		if (usrBtns.rotarySwitch == ROTARY_SWITCH_ON){
			if ((usrBtns.yellowBtn == BTN_PRESSED) && (S.runMode == RUN_OPERATING)){
				...
				//Pause
				uint8_t motors[] = {CALENDER_ROLLER,RD_LEFT_LIFT,RD_RIGHT_LIFT};//{};//,LEFT_LIFT,RIGHT_LIFT};//FLYER,BOBBIN,FR,BR};
				noOfMotors = 3;
			...
			if ((usrBtns.greenBtn == BTN_PRESSED)  && (S.runMode == RUN_PAUSED)){
				...
				//RESUME
				uint8_t motors[] = {CALENDER_ROLLER,RD_LEFT_LIFT,RD_RIGHT_LIFT};//{FLYER,BOBBIN,FR,BR};//,LEFT_LIFT,RIGHT_LIFT};//FLYER,BOBBIN,FR,BR};
				noOfMotors = 3;
